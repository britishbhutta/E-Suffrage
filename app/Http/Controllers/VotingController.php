...
class VotingController extends Controller
{
    ...
    public function step($step, Request $request,)
    {
        ...
        $stepNames = [
            1 => 'Personal Info',
            2 => (auth()->user()->role === 3) ? 'Confirm Conditions Of Partner' : 'Choose Tariff',
            3 => 'Insert Reward',
            4 => 'Detail Of Event',
            5 => 'Payment',
            6 => 'Creation Of Form',
        ...
        $tariffs = null;
        if ($step === 2 || $step === 1 ) {
            $tariffs = Tariff::where('title', '!=', 'Tariff_Unlimited')
    ...
    public function submitVote(Request $request, $token)
    {
        ...
        if(!$votingEvent->booking->media_ad_id){
        ...
        try {
            ...
            $description = $userName . ' Has Casted Vote For Form: ' . $votingEvent->title;
            $action = 'Vote Casted';
            $module = 'Voting';
    ...
    public function complete(Request $request)
    {
        ...
        // save logs
            $userId = Auth::id();
            $userName = Auth::user()->first_name .' '. Auth::user()->last_name;
            $description = $userName . ' Has Created The Form: ' . $booking->votingEvent->title;
            $action = 'Created';
